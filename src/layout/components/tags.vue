<template>
	...
	<transition name="el-zoom-in-top">
		<ul v-if="contextMenuVisible" :style="{left:left+'px',top:top+'px'}" class="contextmenu" id="contextmenu">
			...
			<hr>
			<li @click="closeTabs()" :class="contextMenuItem.meta.affix?'disabled':''"><el-icon><el-icon-close/></el-icon>關閉標籤</li>
			<li @click="closeOtherTabs()"><el-icon><el-icon-folder-delete/></el-icon>關閉其他標籤</li>
			<hr>
			<li @click="maximize()"><el-icon><el-icon-full-screen/></el-icon>最大化</li>
			<li @click="openWindow()"><el-icon><el-icon-copy-document/></el-icon>在新的窗口中打開</li>
		</ul>
	</transition>
</template>
		...
		methods: {
			...
			scrollInit(){
				...
				function handler(event) {
					...
					//火狐上滾鍵值-3 下滾鍵值3，其他內核上滾鍵值120 下滾鍵值-120
