...
<template>
	<slot :open="open">
		<el-button type="primary" plain @click="open">導入</el-button>
	</slot>
	<el-dialog v-model="dialog" title="導入" :width="550" :close-on-click-modal="false" append-to-body destroy-on-close>
			...
			<el-upload ref="uploader"
				...
				<template #tip>
					<div class="el-upload__tip">
						...
						<template v-else>請上傳小於或等於 {{maxSize}}M 的 {{accept}} 格式文件</template>
	...
	export default {
		...
		methods: {
			...
			before(file){
				...
				if (!maxSize) {
					this.$message.warning(`上傳文件大小不能超過 ${this.maxSize}MB!`);
					return false;
				}
				this.loading = true
			},
