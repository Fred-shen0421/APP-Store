		...
		<template v-else>
			<el-empty v-if="myFilter.length<=0" :image-size="100">
				<template #description>
					<h2>沒有常用的過濾</h2>
					<p style="margin-top: 10px;max-width: 300px;">常用過濾可以將多個過濾條件保存為一個集合，方便下次進行相同條件的過濾</p>
				</template>
			</el-empty>
			<ul v-else class="sc-filter-my-list">
				...
				<li v-for="(item, index) in myFilter" :key="index" @click="selectMyfilter(item)">
					...
					<el-popconfirm title="確認刪除此常用過濾嗎？" @confirm="closeMyfilter(item, index)">
						<template #reference>
