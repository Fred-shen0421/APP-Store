...
<template>
	<slot :open="open">
		<el-button type="primary" plain @click="open">導出</el-button>
	</slot>
	<el-drawer v-model="dialog" title="導出" :size="400" direction="rtl" append-to-body destroy-on-close>
		<el-main style="padding: 0 20px 20px 20px;">
			<div v-loading="downLoading" element-loading-text="正在處理中...">
				...
				<el-tabs>
					<el-tab-pane label="常規" lazy>
						<el-form label-width="100px" label-position="left" style="margin: 10px 0 20px 0;">
							<el-form-item label="文件名">
								<el-input v-model="formData.fileName" placeholder="請輸入文件名" />
							</el-form-item>
							<el-form-item label="文件類型">
								<el-select v-model="formData.fileType" placeholder="請選擇文件類型">
									<el-option v-for="item in fileTypes" :key="item" :label="'*.'+item" :value="item" />
								</el-select>
							</el-form-item>
							<slot name="form" :formData="formData"></slot>
						</el-form>
						<el-button v-if="async" type="primary" size="large" icon="el-icon-plus" style="width: 100%;" @click="download" :loading="asyncLoading">發起導出任務</el-button>
						<el-button v-else type="primary" size="large" icon="el-icon-download" style="width: 100%;" @click="download">下 載</el-button>
					</el-tab-pane>
					<el-tab-pane label="列設置" v-if="columnData.length>0" lazy>
						<columnSet :column="columnData"></columnSet>
					</el-tab-pane>
					<el-tab-pane label="其他參數" v-if="data && showData" lazy>
			...
			asyncDownload(apiObj, fileName, data={}){
				...
				apiObj.get(data).then(res => {
					...
					if(res.code == 200){
						...
						this.$msgbox({
							title: "成功發起任務",
							message: `<div><img style="height:200px" src="img/tasks_example.png"/></div><p>已成功發起導出任務，您可以操作其他事務</p><p>稍後可在 <b>任務中心</b> 查看執行結果</p>`,
							type: "success",
						...
						this.$alert(res.message || "未知錯誤", "發起任務失敗", {
							type: "error",
							center: true
						}).catch(() => {})
					}
